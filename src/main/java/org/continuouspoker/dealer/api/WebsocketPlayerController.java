...
public class WebsocketPlayerController {
    ...
    @OnOpen
    public void onOpen(Session session, @PathParam(PARAM_GAME_ID) long gameId,
            @PathParam(PARAM_TEAM_NAME) String teamName) throws ObjectNotFoundException {
        service.registerPlayer(gameId, session, teamName);
    }

    @OnClose
    public void onClose(Session session, @PathParam(PARAM_GAME_ID) long gameId,
            @PathParam(PARAM_TEAM_NAME) String teamName) {
        service.handleWebsocketDisconnect(gameId, session, teamName);
    }

    @OnError
    public void onError(Session session, @PathParam(PARAM_GAME_ID) long gameId,
            @PathParam(PARAM_TEAM_NAME) String teamName, Throwable throwable) {
        log.error("Error received from websocket client with id {}", session.getId(), throwable);
    }

    @OnMessage
    public void onMessage(Session session, @PathParam(PARAM_GAME_ID) long gameId,
            @PathParam(PARAM_TEAM_NAME) String teamName, String message) {
